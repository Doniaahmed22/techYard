...
namespace techYard.Service.Services.CartServices
{
    public class CartService : ICartService
    {
        ...
        public async Task<IReadOnlyList<UserProductsDto>> GetUsersWithProductsAsync()
        {
            ...
            var userProducts = productsInCarts.GroupBy(pc => new { pc.UserId, pc.User.FullName, pc.User.PhoneNumber, pc.User.Email })
                .Select(g => new UserProductsDto
                {
                    UserId = g.Key.UserId,
                    FullName = g.Key.FullName,
                    phoneNumber = g.Key.PhoneNumber,
                    email = g.Key.Email,
                    ProductByUser = g.Select(p => new ProductQuantityDto
                    {
                        ProductId = p.ProductId,
                        Name = p.Product.Name,
                        oldPrice = p.Product.oldPrice,
                        newPrice = (p.Product.oldPrice) - (p.Product.oldPrice * (double)((double)p.Product.discount/100))  ,
